...
'''#Exp 6 portugues lapsbm

with open('Mfcc-Save/lapsbm-test-audios.txt', 'rb') as f:
        XL = pickle.load(f)
...
for i in range(len(XL)):
        if loc[int(XL[i][1])] == 0:#segmento do locutor  já cadastrado
                loc[int(XL[i][1])] =1
                
                X.append([encoding_model.predict([XL[i][0]])[0],XL[i][1]])
        else:#segmentos de testes do locutor
                V.append([encoding_model.predict([XL[i][0]])[0],XL[i][1]])
...
tamanho = len(V)   
print("Experimento 6(LapsBM):")
print("acertou: ", acertou,"de: ",tamanho) 
print('acuracy:',acertou/tamanho)
print('numero de locutores :',len(X) )
print('Segmentos -- Treino:',' Teste:',len(XL),'/n')
...
for q in range(len(pessoas_base3)):
            ...
            V.append(a)
            ins[int(pessoas_base3[q][1])] = ins[int(pessoas_base3[q][1])]+1
...
print("acertou: ", acertou,"de: ",tamanho) 
print('acuracy:',acertou/tamanho)
print('Segmentos -- Treino:',' Teste:',len(pessoas_base3))'''
